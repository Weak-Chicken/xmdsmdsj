...
> 
> For all ```verify``` functions, they need to follow this procedure: First, when called, these functions will need basic router values ```(req, res, next)```, sql connection variables ```(mysqlPool, connection)``` and some optional variables such as ```error```. Second, once called with proper data, the function will perform the verifying process. Finally, when verifying finished, if succeeded, return with no action, let main function continue. If not succeeded, perform following steps: First, sending error messages through given ```res```. Then, close sql connection if there is any. Third, throw an error, with the error infomation, and catch the error outside at main function. Finally, interrupt main function and show errors in console, but not interrupt the whole program.
